...
import { bookSmartFlow } from "@/lib/smart-flow";
...
function LabPage() {
  ...
  const mood = useMemo(() => marketMood(pool, wallets), [pool, wallets]);
  const flow = useMemo(() => bookSmartFlow(pool, wallets), [pool, wallets]);
  const dirty = useMemo(
    () =>
      pool
        .map((t) => ({ t, f: fraudOf(t) }))
        ...
        {dirty.length === 0 ? (
          ...
          dirty.map(({ t, f }) => (
            ...
            >
              ...
              >
                {f.score} {msg(f.tag === "wash" ? "wash" : f.tag === "trap" ? "trap" : f.tag === "bundle" ? "bundleRing" : "spoofVol")}
              </div>
            </Link>
          ))
        )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4">
            {heat.map((c) => (
              <div key={c.cluster} className="border-b border-e border-border px-3 py-2">
                ...
                <div className={cn("font-mono text-xs num", c.avg5m >= 0 ? "text-up" : "text-down")}>
                  {formatPct(c.avg5m)}
                </div>
          ...
          shown.slice(0, 40).map((r) => {
            ...
            return (
              ...
              >
                ...
                <div className={cn("text-end font-mono text-xs num", r.grade === "F" || r.grade === "D" ? "text-down" : "text-up")}>
                  {r.grade}
                </div>
                ...
                <div className="hidden text-end font-mono text-2xs num md:block">{r.volMc.toFixed(2)}</div>
                <div
                  className={cn(
