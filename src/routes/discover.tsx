  ...
  return (
    <div className="mx-auto max-w-6xl px-2 py-3">
      ...
      <div className="overflow-hidden rounded-lg bg-surface shadow-[var(--shadow-border)]">
        <div className="hidden grid-cols-[minmax(0,1.6fr)_repeat(5,minmax(0,1fr))_auto] gap-2 border-b border-border px-3 py-2 font-mono text-2xs text-subtle md:grid">
          <span />
        ...
        {list.length === 0 ? (
          ...
          <Link
            ...
            to="/token/$id"
            params={{ id: t.id }}
            className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 border-b border-border px-3 py-2 hover:bg-elevated/60 md:grid-cols-[minmax(0,1.6fr)_repeat(5,minmax(0,1fr))_auto]"
          >
            ...
            <div className="hidden text-end font-mono text-xs num md:block">{formatUsd(t.vol, 0)}</div>
            <div className="hidden text-end font-mono text-xs num md:block">{formatUsd(t.liq, 0)}</div>
            <div className="hidden text-end font-mono text-xs num md:block">{t.holders}</div>
            <div
              className={cn(
