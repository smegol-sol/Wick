...
import { formatSol, formatTime, formatUsd } from "@wick/core/format";
import type { FlowBias, BookFlow, NameFlow } from "@wick/core/smart-flow";
import { useDesk } from "@/lib/store";
import { cn } from "@/lib/utils";
import type { Msg } from "@/lib/i18n";
import type { Print } from "@wick/core/market";
  ...
  return (
    <span
      className={cn(
        ...
        n > 0.05 ? "text-up" : n < -0.05 ? "text-down" : "text-muted",
      )}
    >
      {n >= 0 ? "+" : ""}
      {formatSol(n)}
      ...
      <div className="overflow-hidden rounded-lg bg-surface shadow-[var(--shadow-border)]">
        <div className="flex items-baseline justify-between border-b border-border px-3 py-2">
          ...
          <span
            className={cn(
              "text-2xs",
              flow.bias === "accumulate"
                ? "text-up"
                : flow.bias === "distribute"
                  ? "text-down"
                  : "text-subtle",
            )}
          >
      ...
      {prints.length === 0 ? (
        ...
        <ul>
          {prints.slice(0, 12).map((p) => (
            ...
            >
              <span
                className={cn("text-xs font-medium", p.side === "buy" ? "text-up" : "text-down")}
              >
