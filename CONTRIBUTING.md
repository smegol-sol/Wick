...
All four of the first commands must pass before a PR is opened; CI runs the same set. They run from the repository root and cover every workspace (`packages/core`, `apps/engine`, `apps/console`). Shared pure logic goes in `packages/core` and is imported as `@wick/core/<module>`; nothing in core may import a browser or React module. The engine's database test runs when `TEST_DATABASE_URL` points at a Postgres 16; CI runs it on TimescaleDB.
