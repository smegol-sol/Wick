...
As built (Phase 2): the followed set is the `wallets` rows with `kind = owner` and `status = follow`, managed from Engine → Followed wallets (`GET/POST /api/wallets`, `POST /api/wallets/:pk/status`, `DELETE /api/wallets/:pk`; following needs the second factor, watching and removing do not). Prints arrive on the log stream the collector already subscribes to, not a separate webhook: the collector books the print and hands it to the decision loop with the moment it was seen. A buy becomes an entry intent of the `mirror-follow` rule at `sizeMul` of the normal size through all six gates; a sell closes our whole position in that mint through the quote gate; a print seen more than `maxCopyGapMs` after its block time is counted (`wick_mirror_prints_total{outcome="stale"}`) and not copied. Every copy writes an `events` row (`component = decision`, `msg = copy`, with the wallet, the signature, the gap and the intent id), which is where the wallet panel and the demotion read from. Demotion: once a day the evaluator takes the last ten copied buys with a measured 30-minute outcome; a negative mean sends the wallet back to `watch` with the reason on the row and a line to the owner (`wick_wallet_demotions_total`), and only the owner follows it again. The gap and the slippage are inside those outcomes because they are measured from our fill, so the ENGINE rule "gap plus slippage over profit" is that mean.
