...
| caddy             | reverse proxy bound to the Tailscale IP only         | your tailnet devices |
...
8. **Start.** `docker compose up -d --build`. The engine applies migrations on boot. `docker compose logs -f engine` should show `listening` and then `snapshots` counters moving on `http://<tailscale-ip>/metrics` (send `Authorization: Bearer <DASHBOARD_TOKEN>`).
9. **Prove the alerts.** Stop the engine for two minutes (`docker compose stop engine`) and confirm `EngineDown` arrives on Telegram and the healthchecks.io check goes red, then start it again. Phase 1's exit condition requires one test alert per rule to have reached the phone.
