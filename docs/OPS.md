...
- **Upgrade:** `cd /opt/wick/apps/engine/deploy && ./update.sh` pulls `main`, builds the console, stamps `WICK_COMMIT`, rebuilds the engine and Prometheus, waits for `/healthz` and prints the migration and bot lines from the log (`./update.sh <ref>` deploys a branch or commit for a hotfix under test). The engine boots sealed: the decision layer runs and writes intents, but nothing executes until the vault is unsealed.
...
- **Devnet run** (ROADMAP Phase 2): `DEVNET_RPC_URL=https://api.devnet.solana.com npm -w @wick/engine test` runs the adapter and, with `TEST_DATABASE_URL`, the executor against devnet. The public faucet refuses most datacenter addresses; from the laptop it usually pays, otherwise fund a throwaway devnet key at faucet.solana.com and pass it as `DEVNET_SECRET_B58` (64 bytes, base58; a devnet key only, never the execution wallet's). The tests skip, never fail, when neither is available.
- **Followed wallets:** Engine → Followed wallets: paste the public key, a label and the six-digit code, then Follow; at most six (the mirror rule's `maxWallets`). Every print of a followed wallet becomes a `mirror-follow` intent through the full gate chain at half size while the rule is in shadow, so nothing executes; the panel shows copies, the mean 30-minute return of the last ten and the last copy. Watch and Remove need no code. A wallet the evaluator demoted shows the reason and stays on watch until you follow it again. By API: `POST /api/wallets` with `{"pk", "label", "code"}`, `POST /api/wallets/<pk>/status` with `{"status": "watch"}` (or `"follow"` plus `code`), `DELETE /api/wallets/<pk>`.
...
| SourceStale pump.fun        | `logs engine \| grep 'source answered nothing'` names the reason (`http 429`, `http 403`, `timeout`, `body`); `wick_source_failures_total{source="pump.fun",reason}` counts it. 429 and 403 back off on their own (Retry-After, 30 s or 60 s); a block that outlasts an hour needs a different discovery path, not a retry |
...
- **RPC cut** (`./drill.sh rpc-cut`): the RPC host's addresses are rejected in Docker's `DOCKER-USER` iptables chain for 90 s (a `ufw deny out` never sees a container's forwarded traffic; the first run on 2026-09-08 proved that with the RPC answering through the block); expect a self-halt on `source rpc-primary stale` with `slotLag` null (reads keep flowing through the public fallbacks, so `rpc` itself stays fresh; the second run on 2026-09-08 showed exactly that and no halt, which is why the configured endpoint is its own source now), the engine still up, and the halt cleared within a minute of the block lifting.
- **Postgres stopped** (`./drill.sh db-stop`): `docker compose stop db` for 60 s; expect the engine's restart count unchanged (the first run on 2026-09-08 found the process dying with the database: an idle pool client's `error` had no listener; `/healthz` answered nothing while Docker restarted it and the migrations failed against a stopped database), `dbOk=false` and a self-halt on `/healthz`, DbErrors counted, and `dbOk=true` with `self-halt cleared` in the log once it is back.
