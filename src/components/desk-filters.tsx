...
function presetMatches(id: Preset, s: DeskSettings): boolean {
  ...
  return (
    ...
    s.minMc === p.minMc &&
    s.maxMc === p.maxMc &&
    s.maxBundled === p.maxBundled &&
    s.maxDev === p.maxDev &&
    s.maxSnipers === p.maxSnipers &&
    ...
    <div className="flex flex-col gap-2 px-3 py-2">
      ...
      <label className="flex flex-col gap-1 text-2xs text-muted">
        ...
        <Input
          ...
          placeholder="liq>=2k mc<=80k age<20 freeze=0 topic!=ai"
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
        ...
        />
        <NumField
          label={msg("minHolders")}
          value={settings.minHolders}
          onChange={(n) => patch({ minHolders: n })}
        />
        <NumField
          label={msg("maxBundled")}
          value={settings.maxBundled}
          onChange={(n) => patch({ maxBundled: n })}
        />
        <NumField
          label={msg("maxDev")}
          value={settings.maxDev}
          onChange={(n) => patch({ maxDev: n })}
        />
        <NumField
          label={msg("maxSnipers")}
          value={settings.maxSnipers}
          onChange={(n) => patch({ maxSnipers: n })}
        />
        <label className="flex flex-col gap-1 text-2xs text-muted">
          {msg("minGrade")}
          <select
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-border pt-2 font-mono text-2xs text-muted">
        ...
        <span>{msg("guardRpc")}</span>
        <span>{msg("guardStore")}</span>
